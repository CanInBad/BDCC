...
func run(_triggerID, _args):
	var curCustomSize = Util.cmToString(GM.main.getFlag("Hypertus.HyperPenisCustomSize", 0))
	if (not GM.pc.getBodypart(BodypartSlot.Penis).getTraits() == null) and ("PartTrait.Hyperable" in GM.pc.getBodypart(BodypartSlot.Penis).getTraits()) and (GM.main.getCurrentScene().saveData()["pickedAttribID"] == "cocksize"):
